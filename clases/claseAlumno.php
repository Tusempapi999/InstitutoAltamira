<?php
    ...
    class alumno extends user { // Se crea la clase alumno que hereda de la clase User
        ...
        public function Listar_alumnos($grupo_id) { // Función para listar un alumno con una matricula específica 
             $this->sentencia = "SELECT nombre FROM user 
                                INNER JOIN alumno ON user.id = alumno.user_id 
                                INNER JOIN matriculado ON alumno.id = matriculado.alumno_id 
                                WHERE matriculado.grupo_id = '$grupo_id'"; 
                                //Muestrame el nombre de la tabla user uniendome
                                // alumno donde el id de user sea igual al user_id de alumno y uniendome la tabla matricula 
        ...
        public function Listar_profesores($alumno_id) { // Función para listar profesores
            $this->sentencia = "SELECT user.nombre, profesor.especialidad, asignatura.nombre AS asignatura 
                                FROM user 
                                INNER JOIN profesor ON user.id = profesor.user_id 
                                ...
                                // uniendo la tabla user con profesor donde el id de user sea igual al user_id de profesor
